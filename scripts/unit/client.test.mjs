...
test('transmits an expected-actor fence for authenticated writes, not credentials', async () => {
  let headers;
  const actor = '22222222-2222-4222-8222-222222222222';
  const client = createMutationClient({ fetchFn: async (_, options) => {
    headers = new Headers(options.headers);
    return Response.json({ id: '11111111-1111-4111-8111-111111111111' });
  }});
  await client.send(actor, '/api/learning', 'submit', { reason: '개인 원문' });
  assert.equal(headers.get('x-science-actor'), actor);
