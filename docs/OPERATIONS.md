...
- 수업 채팅: 실제 Cursor 답변, 등록 자료의 정확한 원문 인용 검증. 최대 3개 자료의 앞 3,000자씩 제공하므로 긴 문서의 후반 근거는 현재 누락될 수 있습니다. 자료에 근거가 없으면 찾지 못했다고 안내합니다.
...
확장 검증: `pnpm test:classroom`. 실제 등록 자료 사용 여부, 다른 반 접근 거부, 피드백 읽음, 토론 답글·공감 중복 요청, Cursor 근거 인용 답변, 단원에 맞는 활동, 관찰 기록 입력을 검증합니다. 검증 데이터는 ‘연동 검증’ 이름으로 표시됩니다.
