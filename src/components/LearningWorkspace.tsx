...
export default function LearningWorkspace(){
 const [view,setView]=useState<View>(empty),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState('');
 ...
 const refresh=useCallback(async()=>{
  const request=latest.current.begin();
  try{
   ...
   if(knownUser.current && knownUser.current!==home.user_id){course.current='';cursor.current=null;setSelected('');setView(empty);}
   ...
   if(!active){setView({...empty,user_id:home.user_id});return true;}
   ...
   setView({...data,courses:home.courses});return true;
  }catch(e){
   if(request.isCurrent()&&mounted.current){
    if((e as {status?:number}).status===401){knownUser.current='';course.current='';cursor.current=null;setView(empty);}
    else setError(e instanceof Error?e.message:'수업 연결을 확인해 주세요.');
   }return false;
  }finally{if(request.isCurrent()&&mounted.current)setLoading(false);}
 },[]);
 useEffect(()=>{
 ...
 const act:Act=async(action,data,id,version)=>{
  ...
  try{
   await client.current.send(knownUser.current,'/api/learning',action,data,id,version??null);
   if(!mounted.current)return true;
   if(action==='submit')cursor.current=null;
 ...
 return <main className="learning-shell">
  ...
  {loading?<p role="status">수업을 불러오고 있어요.</p>:!view.user_id?<section className="learning-card"><h2>수업에 들어가기</h2>
   ...
   <form onSubmit={e=>{e.preventDefault();const form=new FormData(e.currentTarget);void authenticate('login',{email:form.get('email'),password:form.get('password')});}}>
    ...
    <button disabled={busy}>로그인</button></form></section>:<>
   <div className="learning-toolbar"><label>수업<select value={view.course_id} disabled={busy} onChange={e=>{
   ...
   <button disabled={busy} onClick={()=>void refresh()}>다시 불러오기</button></div>
   {!view.courses.length?<p>아직 등록된 수업이 없어요. 담당 관리자에게 수업 등록을 요청하세요.</p>:<div key={view.user_id+view.course_id}>
    ...
    <section className="learning-card"><h2>{view.role==='teacher'?'학생 생각 검토':'내 학습 기록'}</h2>
     ...
     {!view.attempts.length?<p>아직 기록이 없어요.</p>:<>
      <label>기록 선택<select value={current?.id??''} onChange={e=>setSelected(e.target.value)} disabled={busy}>{view.attempts.map(a=><option key={a.id} value={a.id}>{a.title} · {stages[a.stage]} · {new Date(a.created_at).toLocaleString('ko-KR',{timeZone:'Asia/Seoul'})} · {a.student_id.slice(0,8)}</option>)}</select></label>
      {current&&<AttemptPanel key={view.user_id+current.id+':'+current.version} row={current} teacher={view.role==='teacher'} act={act} busy={busy}/>}</>}
     <div className="learning-toolbar"><button disabled={busy||!cursor.current} onClick={()=>{cursor.current=null;setSelected('');void refresh();}}>최신 기록</button>
      <button disabled={busy||!view.has_more} onClick={()=>{const last=view.attempts.at(-1);if(last){cursor.current={before:last.created_at,before_id:last.id};setSelected('');void refresh();}}}>이전 기록</button></div>
    </section>
   </div>}
  </>}
 </main>;
...
function PackReview({pack:p,busy,assigned,activate}:{pack:Pack;busy:boolean;assigned:boolean;activate:()=>Promise<boolean>}){
 ...
 return <details><summary>{p.title} · {p.version}{assigned?' · 배정됨':''}</summary><p>{p.reviewNotice}</p><h3>범위</h3><p>{p.scope}</p><h3>문항</h3><p>{p.prompt}</p><p>정답: {p.correctAnswer}</p><h3>검토 기준</h3><p>{p.rubric}</p>
  ...
  <label className="learning-check"><input type="checkbox" checked={reviewed} onChange={e=>setReviewed(e.target.checked)}/>내용과 수업 적합성을 검토했습니다.</label>
...
function AttemptPanel({row:r,teacher,act,busy}:{row:Attempt;teacher:boolean;act:Act;busy:boolean}){
 ...
 return <article><h3>{r.title} · {stages[r.stage]}</h3><p>{r.prompt}</p><blockquote>{r.response.answer}<br/>{r.response.reason}</blockquote>
  ...
  {teacher&&r.stage==='awaiting_review'&&<section><label>검토 의견 / 학생에게 보낼 질문<textarea required value={feedback} maxLength={2000} onChange={e=>setFeedback(e.target.value)}/></label>
   <label>확인 활동<select value={activity} onChange={e=>setActivity(e.target.value)}>{r.activities?.map(a=><option key={a.id} value={a.id}>{a.title}</option>)}</select></label>
   <div className="learning-toolbar"><button disabled={busy||!feedback.trim()} onClick={()=>void run('request_clarification',{feedback})}>이유 더 묻기</button>
    <button disabled={busy||!feedback.trim()||!activity} onClick={()=>void run('assign_activity',{feedback,activity_id:activity})}>활동 배정</button>
  ...
  {!teacher&&r.stage==='activity_assigned'&&<form onSubmit={e=>{e.preventDefault();void run('complete_activity',{observation,explanation});}}>
   <label>활동에서 확인한 내용<textarea required maxLength={2000} value={observation} onChange={e=>setObservation(e.target.value)}/></label>
   <label>다시 설명한 내 생각<textarea required maxLength={2000} value={explanation} onChange={e=>setExplanation(e.target.value)}/></label>
