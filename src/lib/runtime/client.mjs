...
export function createMutationClient({ fetchFn = fetch, storage = () => null, timeoutMs = 15_000, maxPending = 64 } = {}) {
  ...
  /** @param {string} endpoint @param {Record<string, unknown>} body @param {string | null} [actor] */
  async function transmit(endpoint, body, actor = null) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
      const response = await fetchFn(endpoint, {
        method: "POST", credentials: "same-origin", cache: "no-store",
        headers: { "Content-Type": "application/json", ...(actor ? { "X-Science-Actor": actor } : {}) }, body: JSON.stringify(body), signal: controller.signal,
      });
      let result;
  ...
  async function send(scope, endpoint, action, data, id = null, version = null) {
    ...
    const task = (async () => {
      try {
        const result = await transmit(endpoint, { ...command, request }, scope);
        forget(key);
        return result;
      } catch (error) {
